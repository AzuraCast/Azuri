...
export default {
    ...
    execute: (client, serverData, message, ...args) => {
        if(!message.guild) return message.reply(L._U(serverData.locale, 'server_only'));
        if(!args.length) return message.reply(L._U(serverData.locale, 'missing_args'));
        ...
        switch(args[0][0]) {
            ...
            case 'home':
                if(args[0][1]) {
                    ...
                    if(!channel) return message.reply(L._U(serverData.locale, 'no_find_voice'));
                    if(channel.type !== 'voice') return message.reply(L._U(serverData.locale, 'channel_not_voice'));
                    ...
                    return message.reply(`${L._U(serverData.locale, 'current_homeroom')}: ${serverData.home}`)
                }
            break;
            ...
            case 'prefix':
                if(args[0][1]) {
                    ...
                    return message.reply(`${L._U(serverData.locale, 'current_prefix')}: ${serverData.preifx}`)
                }
            break;
            ...
            case 'timezone':
                if(args[0][1]) {
                    ...
                    return message.reply(`${L._U(serverData.locale, 'current_timezone')}: ${serverData.timezone} | ${L._U(serverData.locale, 'full_list_timezone')} <https://en.wikipedia.org/wiki/List_of_tz_database_time_zones>`)
                }
            break;

            case 'permission':
                if(args[0][1]) {
                    if(!(client.commands.get(args[0][2]) || client.commands.find(cmd => cmd.aliases && cmd.aliases.includes(args[0][2])))) return message.reply(L._U(serverData.locale, 'no_find_command')); 
                    if(args[0][1] == 'role') {
                        if(message.guild.roles.cache.filter(role => role.id === args[0][3])) {
                            ...
                            return message.reply(L._U(serverData.locale, 'no_find_role'))
                        }
                        ...
                        if(message.guild.members.cache.get(args[0][3])) {
                            ...
                            return message.reply(L._U(serverData.locale, 'no_find_user'))
                        }
                    ...
                    return message.reply(L._U(serverData.locale, 'missing_args'));
                }
            ...
            default:
                let prefix = serverData.prefix ? serverData.prefix : process.env.DEFAULT_PREFIX;
